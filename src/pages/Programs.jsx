...
import { Box, Sphere, Cone, Torus, OrbitControls, Float } from '@react-three/drei';
import * as THREE from 'three';

// 3D Program Icons Component
const ProgramIcons3D = () => {
  const iconsRef = useRef();
  
  useFrame((state) => {
    const time = state.clock.elapsedTime;
    if (iconsRef.current) {
      iconsRef.current.rotation.y = time * 0.1;
      iconsRef.current.position.y = Math.sin(time * 0.5) * 0.2;
    }
  });
  ...
  return (
    <group ref={iconsRef}>
      ...
      <Float speed={1.5} rotationIntensity={0.8} floatIntensity={0.3}>
        <Sphere args={[0.8, 32, 32]} position={[3, -1, -1]}>
          <meshStandardMaterial 
            color="#10b981" 
            roughness={0.2} 
            metalness={0.9}
      ...
      <Float speed={2.5} rotationIntensity={1.2} floatIntensity={0.4}>
        <Cone args={[0.6, 1.5, 8]} position={[0, 3, -3]}>
          <meshStandardMaterial 
            color="#ef4444" 
            roughness={0.2} 
            metalness={0.8}
      ...
      <Float speed={1.8} rotationIntensity={0.6} floatIntensity={0.6}>
        <Torus args={[0.8, 0.3, 16, 100]} position={[-2, -2, 0]}>
          <meshStandardMaterial 
            color="#8b5cf6" 
            roughness={0.3} 
            metalness={0.7}
